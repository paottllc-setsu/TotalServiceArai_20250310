...
Public Class 工事マスタ一覧

    Private Sub 工事マスター編集_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub DataGridLoad()
        Try
            ...
            Using Conn As New MySqlConnection(メインメニュー.Builder.ToString)
                ...
                Dim sql As String = "SELECT * FROM subjects_master ORDER BY subjectNo, slaveNo"
                ...
                Using result As MySqlDataReader = command.ExecuteReader
                    ...
                    While result.Read()
                        ...
                        DataGridView1.Rows(row).Cells(0).Value = result.GetInt16(0)
                        DataGridView1.Rows(row).Cells(1).Value = result.GetInt16(1)
                        DataGridView1.Rows(row).Cells(2).Value = result.GetString(2)
                        DataGridView1.Rows(row).Cells(3).Value = result.GetString(3)
                        DataGridView1.Rows(row).Cells(4).Value = result.GetInt16(4)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Dim fm As New 工事マスタ編集(0, param) With {
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Dim fm As New 工事マスタ編集(2, param) With {
            .StartPosition = FormStartPosition.CenterParent
        }
